...
define(
    ...
    function(Backbone, _) {
        ...
        var _allChecked = new Backbone.Model( {checked: false, length: 0} );
        // model for the CountTasksWidget
        var _countTaskModel = new Backbone.Model( {itemLeft:0, anyItemSelected: false, itemSelected: 0, length: 0} );
        //url for _tasks collection
        _tasks.url = 'http://localhost:8081/toDoItems';
        var _repository = {
            //for recalculating and syncing _countTaskModel and _allChecked
            syncAll: function() {
                this.SyncCountTaskModel();
                this.syncAllChecked();
            },
            // Sets the value of all attributes of _countTaskModel
            SyncCountTaskModel: function() {
                var filtered = _tasks.where( {completed : false} );
                _countTaskModel.set('length', (_tasks.length));
                _countTaskModel.set('itemSelected', (_tasks.length-filtered.length));
                _countTaskModel.set('itemLeft', filtered.length);
                _countTaskModel.set('anyItemSelected', (filtered.length < _tasks.length));
            },
            // return _countTaskModel
            getCountTaskModel: function () {
                return _countTaskModel;
            },
            // return _allChecked
            ...
            // Sets the value of all attributes of _allChecked
            syncAllChecked: function() {
                _allChecked.set('length', _tasks.length);
                var filtered = _tasks.filter(function(m) {
                    return m.get('completed') === false;
                });
                _allChecked.set('checked', filtered.length?false:true);
            },
